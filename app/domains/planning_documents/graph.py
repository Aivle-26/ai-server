...
from .chunk_selector import select_analysis_inputs
...
class PlanningDocumentGraph:
    ...
    def _select_analysis_inputs(
        ...
        selection = select_analysis_inputs(
            ...
            max_analysis_chunks=(
                self.settings.planning_max_analysis_chunks
            ),
        )
        selected_inputs = [
            {
                "document_id": self._safe_document_id(
    ...
    def _extract_with_llm(self, state: PlanningDocumentState) -> dict[str, Any]:
        if perf_counter() >= state["deadline_monotonic"]:
            ...
            outcome = self.llm_service.extract_with_metrics(
                ...
                fallback_extractions=(
                    state["selected_fallback_extractions"]
                ),
                request_id=state["request_id"],
            ...
            if not isinstance(outcome, PlanningLLMExtractionOutcome):
                raise TypeError(
                    "extract_with_metrics returned an invalid outcome"
                )
        ...
        unselected_text_count = (
            len(state["chunks"])
            - len(state["selected_chunk_indices"])
        )
        return {
            "partial_extractions": [
    ...
    def _safe_document_id(self, file_name: str) -> str:
